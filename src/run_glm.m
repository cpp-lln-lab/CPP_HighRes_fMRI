...
for i = 1:size(events_files, 1)
  confounds = spm_select('FPList', ...
                         output_dir, ...
                         ['^rp_sub-pilot001_ses-' opt.ses '_task-' opt.task '_run-00' num2str(i) '_' contrast '.txt$']);
  ...
  switch contrast
    ...
    case 'bold'
      ...
      image = spm_select('FPList', ...
                         output_dir, ...
                         ['^' prefix 'sub-pilot001_ses-' opt.ses '_task-' opt.task '_run-00' num2str(i) '_' contrast '.nii$']);

    case 'vaso'

      image = spm_select('FPList', ...
                         output_dir, ...
                         ['^' prefix 'sub-pilot001_ses-' opt.ses '_task-' opt.task '_run-00' num2str(i) '_' contrast '_VASO_LN.nii$']);
